...
from deepLearningVolatility.stochastic.heston import SpotVarianceTuple
...
def generate_rough_bergomi(
    ...
    if antithetic:
        # For multivariate normal, we need special handling
        n_base = (n_paths + 1) // 2
        n_anti = n_paths - n_base
        dW1_base = _dW1_generator.sample(torch.Size([n_base, n_steps - 1]))
        
        if n_anti > 0:
            dW1 = torch.cat([dW1_base, -dW1_base[:n_anti]], dim=0)
        else:
            dW1 = dW1_base
        
        dW2 = randn_antithetic(n_paths, n_steps - 1, dtype=dtype, device=device) * math.sqrt(dt)
    else:
        dW1 = _dW1_generator.sample(torch.Size([n_paths, n_steps - 1]))
        dW2 = torch.randn([n_paths, n_steps - 1], dtype=dtype, device=device) * math.sqrt(dt)
