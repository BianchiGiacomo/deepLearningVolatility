...
def generate_kou_jump(
    ...
    if antithetic:
        returns = randn_antithetic(n_paths, n_steps, dtype=dtype, device=device) * math.sqrt(dt) * sigma
    else:
        returns = engine(*(n_paths, n_steps), dtype=dtype, device=device) * math.sqrt(dt) * sigma
    returns[:, 0] = 0.0
