...
def generate_merton_jump(
    ...
    if antithetic:
        jump_randn = randn_antithetic(n_paths, n_steps - 1, dtype=dtype, device=device)
    else:
        jump_randn = engine(*(n_paths, n_steps - 1), dtype=dtype, device=device)
    
    jump = (
        jump_mean * n_jumps
        + jump_randn * jump_std * n_jumps.sqrt()
    )
    jump = torch.cat(
    ...
    if antithetic:
        randn = randn_antithetic(n_paths, n_steps, dtype=dtype, device=device)
    else:
